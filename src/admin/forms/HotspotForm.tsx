...
const Button = styled.button<{ $danger?: boolean; $active?: boolean }>`
  border-radius: 12px;
  border: 1px solid ${(p) =>
    p.$danger ? p.theme.errorBorder : p.$active ? p.theme.accent : p.theme.buttonBorder};
  background: ${(p) =>
    p.$danger ? p.theme.errorSoft : p.$active ? p.theme.accentSoft : p.theme.buttonBg};
  color: ${(p) =>
    p.$danger ? p.theme.error : p.$active ? p.theme.accent : p.theme.text};
  padding: 10px 12px;
  cursor: pointer;
  font-weight: 700;
  white-space: nowrap;
  ...
  &:hover {
    background: ${(p) =>
      p.$danger ? p.theme.errorBorder : p.$active ? p.theme.accentSoft : p.theme.buttonHover};
  }
...
const Rect = styled.button<{ $x: number; $y: number; $w: number; $h: number; $active?: boolean }>`
  ...
  border-radius: 8px;
  border: 2px solid ${(p) => p.$active ? p.theme.accent : `${p.theme.accent}88`};
  background: ${(p) => p.$active ? `${p.theme.accent}28` : `${p.theme.accent}14`};
  box-shadow: ${(p) => p.$active ? `0 0 0 3px ${p.theme.accentSoft}` : "0 0 0 2px rgba(0,0,0,0.15)"};
